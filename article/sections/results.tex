...
consistency (within-item SD), on the same convention as Fig.~\ref{fig:rq1}: points below zero became
less consistent. Most points sit low and near the vertical axis, so reasoning usually costs
consistency while moving agreement little. PT-CS points use the verified stratum.}
\label{fig:rq1trade}
\end{figure}
...
\begin{figure*}[t]
\centering
\input{figures/fig_rq1_panels.pgf}
...
more. PT-CS uses the verified stratum.}
\label{fig:rq1}
\end{figure*}

Agreement gains are specific to the model and the dataset, never uniform: on the trade-off plane of
Fig.~\ref{fig:rq1trade} the points spread widely along the horizontal agreement axis (dQWK is the
...
cell's reasoning-off QWK): Mohler (dQWK $+0.071$, $+11$\%), SemEval ($+0.098$, $+18$\%), RIAYN
...
keys its code guidance on baseline behaviour rather than model identity. We saw the collapse in one
model on one serving backend, so we treat it as illustrative rather than established.
...
\begin{figure}[t]
\centering
\input{figures/fig_rq2_guidance.pgf}
\caption{Evaluation-guidance benefit per dataset: the change in agreement (dQWK, K=5, with 95\%
confidence intervals) when the reference answer or rubric is added on top of the question and student
answer. Qwen3.5, reasoning off. PT-CS code is the verified stratum.}
\label{fig:rq2}
\end{figure}
...
Overall, evaluation guidance is a low-cost and reliable gain when the reference is sound, lifting
...
\begin{figure}[!ht]
\centering
\input{figures/fig_rq3_scope_decomp.pgf}
\caption{Scope and decomposition contrasts on PT-CS-verified code (dQWK, K=5, with 95\% confidence
...
\begin{figure}[!ht]
\centering
\input{figures/fig_rq4_cost.pgf}
...
\begin{figure}[!b]
\centering
\input{figures/fig_rq6_transfer.pgf}
