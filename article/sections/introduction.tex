...
thinking improves general LLM-as-a-judge accuracy~\cite{jayarao_explicit_2025}, but its effect depends
on the task and the architecture~\cite{larionov_deepseek_2025}. Long reasoning traces also carry
verbose, redundant computation~\cite{sui_stop_2025}, and no peer-reviewed study isolates its effect on
...
fourth, PT-CS, is an export of a real Portuguese university grading deployment built by the
authors; we use it to test whether the public-dataset guidance transfers to a deployment context, and
...
The contribution is threefold. First, a configuration decision guide for LLM grading in CS, keyed by
