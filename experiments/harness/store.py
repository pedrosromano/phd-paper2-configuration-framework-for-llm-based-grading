...
def build_run_row(item, config, result, prompt: str) -> dict:
    ...
    return {
        ...
        # --- reproducibility bundle (CLAUDE.md §8) ---
        "quant": meta["quant"], "temperature": config.temperature,
        "prompt_hash": prompt_hash(prompt), "k": config.k,
        "ts": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        # --- result (raw never discarded) ---
...
def run_one(item, config, grade_fn, store: RunStore) -> tuple[dict, bool]:
    ...
    row = build_run_row(item, config, result, prompt)
    store.append(row)
    return row, False
